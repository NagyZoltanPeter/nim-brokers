...
static void runScenario(const char* name, IterOut (*fn)(const RunCfg&),
    ...
    std::printf("── %s — %d B payload, %d msgs/thread (median of %d) ──────\n",
                name, static_cast<int>(payload.size()), perThread, iters);
    ...
    for (int k : threadCounts) {
        ...
        std::printf("  %-9d%-11llu%-13.0f%-11.2f%-12s%llu\n", k,
                    static_cast<unsigned long long>(
                        static_cast<uint64_t>(k) * static_cast<uint64_t>(perThread)),
                    med, med * static_cast<double>(payload.size()) / 1e6,
...
int main() {
    ...
    std::printf("# benchlib FFI e2e throughput — payload=%dB handler=fnv1a64\n\n",
                payloadSize);
    runScenario("signal C++ -> Nim (one-way _call, drain-timed)", runSignal,
                threadCounts, perThread, iters, payload, perMsgHash);
    runScenario("async request C++ -> Nim -> C++ (_callAsync turnaround)", runAsync,
                threadCounts, perThread, iters, payload, perMsgHash);
    runScenario("sync request C++ -> Nim -> C++ (blocking _call turnaround)", runSync,
                threadCounts, perThread, iters, payload, perMsgHash);

    std::printf("  correctness: all counts and hashes matched expectations.\n");
    return 0;
}
