...
proc generateCborCppHeaderFile*(
  ...
  # Enum json_type_traits specialisations — manual because jsoncons
  # doesn't ship a numeric-enum macro that we can reuse without naming
  # every case. We encode/decode through the underlying int32_t to
  # match the BrokerCbor `EnumAsNumber` wire format.
  if enumNames.len > 0:
    h.add(
