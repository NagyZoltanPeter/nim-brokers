...
proc generateCMakePackageFiles*(
  ...
  let pkgMajor = if semverParts.len >= 1: semverParts[0] else: "0"
  var versionFile = ""
  ...
  if hasCpp:
    ...
    if cborMode:
      cfg.add("#       (depends on jsoncons; set " & upperName &
        "_JSONCONS_INCLUDE_DIR to override discovery)\n")
  cfg.add("\n")

  cfg.add("cmake_minimum_required(VERSION 3.16)\n\n")

  cfg.add("get_filename_component(_" & libName &
    "_pkg_dir \"${CMAKE_CURRENT_LIST_DIR}\" ABSOLUTE)\n\n")

  # Resolve platform-specific shared library filename.
  cfg.add("if(WIN32)\n")
  cfg.add(
    "  set(_" & libName & "_shared_name \"" & libName &
      ".dll\")\n"
  )
  cfg.add(
    "  set(_" & libName & "_import_name \"" & libName & ".lib\")\n"
  )
  cfg.add("elseif(APPLE)\n")
  cfg.add(
    "  set(_" & libName & "_shared_name \"lib" & libName & ".dylib\")\n"
  )
  cfg.add("else()\n")
  cfg.add(
    "  set(_" & libName & "_shared_name \"lib" & libName & ".so\")\n"
  )
  cfg.add("endif()\n\n")

  cfg.add(
    "set(_" & libName & "_shared_path \"${_" & libName & "_pkg_dir}/${_" &
      libName & "_shared_name}\")\n"
  )
  cfg.add("if(NOT EXISTS \"${_" & libName & "_shared_path}\")\n")
  cfg.add(
    "  message(FATAL_ERROR \"" & libName &
      ": shared library not found at '${_" & libName & "_shared_path}'.\")\n"
  )
  cfg.add("endif()\n\n")

  cfg.add(
    "set(_" & libName & "_header_path \"${_" & libName & "_pkg_dir}/" &
      libName & ".h\")\n"
  )
  cfg.add("if(NOT EXISTS \"${_" & libName & "_header_path}\")\n")
  cfg.add(
    "  message(FATAL_ERROR \"" & libName & ": C header not found at '${_" &
      libName & "_header_path}'.\")\n"
  )
  cfg.add("endif()\n\n")

  # IMPORTED SHARED target — the C-level surface. Carries headers + library.
  cfg.add(
    "if(NOT TARGET " & nsName & "::" & libName & ")\n"
  )
  ...
  cfg.add("  if(WIN32)\n")
  cfg.add(
    "    set(_" & libName & "_import_path \"${_" & libName & "_pkg_dir}/${_" &
      libName & "_import_name}\")\n"
  )
  ...
  if hasCpp:
    ...
    cfg.add(
      "  add_library(" & nsName & "::" & libName & "_cpp INTERFACE IMPORTED)\n"
    )
    cfg.add(
    ...
    if cborMode:
      ...
      cfg.add(
        "  if(NOT DEFINED " & upperName & "_JSONCONS_INCLUDE_DIR)\n"
      )
      cfg.add(
      ...
      cfg.add("  endif()\n")
      cfg.add(
        "  if(NOT " & upperName & "_JSONCONS_INCLUDE_DIR)\n"
      )
      cfg.add(
        "    message(FATAL_ERROR \"" & libName &
      ...
      cfg.add(
        "  set_property(TARGET " & nsName & "::" & libName &
          "_cpp APPEND PROPERTY INTERFACE_INCLUDE_DIRECTORIES \"${" &
          upperName & "_JSONCONS_INCLUDE_DIR}\")\n"
      )
    cfg.add("endif()\n\n")
