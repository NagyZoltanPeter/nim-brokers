...
Status: **IMPLEMENTED** (2026-05-21, `retire-native-cbor-optimize`).
Replaces the §3 sketch in `doc/CBOR_Refactoring_Round2.md`. Companion:
`doc/Event_Dispatch_Options.md` (options-survey doc).

> **Shipped commit chain on `retire-native-cbor-optimize`:**
>
> | Phase | Commit | Scope |
> |---|---|---|
> | D-1 + D-2 | `787f452` | Restore delivery thread; move event handlers to it |
> | D-3 | `29cb8f5` | Event courier ring + atomic-counter fast path + dropAllListeners hook |
> | D-4 | `14220ce` | Three-lane stress drivers (mixed_audience / no_foreign / no_nim) |
> | D-5 | `963e424` | Shutdown drain + slow-callback stress + ASAN nimble task |
> | D-6 | `990b3fd` | Event-dispatch microbench + baseline numbers |
> | D-7 | (this commit) | Doc sweep |
>
> Headline numbers (Apple M4 / `-d:release --mm:orc`, see
> `doc/bench_baseline.md` § "Event dispatch — Part D-6"):
>
> | scenario | ns/emit |
> |---|---:|
> | no foreign + no nim (atomic-counter fast path) | **~615** |
> | + 1 foreign subscriber (full courier path)     | **~975** (+360) |
> | + 8 foreign subscribers (encode amortized)     | **~895** |
> | + 4 same-thread Nim listeners (Lane 1 only)    | **~1170** |
...
| 5 | **D-5 Shutdown drain + ASAN lifecycle stress + slow-callback proof** [shipped] | Added `stress_event_shutdown.cpp` (mid-stream `<lib>_shutdown(ctx)` with concurrent emit + addRequest workers + foreign callbacks; 4 cycles per run) and `stress_event_slow_callback.cpp` (foreign callback sleeps 100 ms × K_EMITS; asserts addRequest throughput ratio ≥ 0.5× baseline — empirical proof the delivery-thread restoration was needed). Run under non-ASAN, ASAN+`--mm:orc`, ASAN+`--mm:refc` via `nimble runFfiBenchEventStress` + `nimble runFfiBenchEventStressAsan`. **The `shuttingDown: Atomic[int]` flag from the original scope was NOT needed** — the existing `e.active` check inside the per-event handler (under `ctxsLockIdent`) already serializes correctly with `_shutdown`'s `active = false` write; the shutdown test passes ASAN-clean in both memory managers without it. Valgrind run is left as a follow-up (ASAN catches the same lifecycle bugs on macOS where valgrind isn't viable). | 1.5 d |
| 6 | **D-6 Bench harness extension** [shipped] | Added `test/ffibench/bench_event_driver.cpp` measuring four scenarios (a/b/c/d), driven by `nimble runFfiBenchEvent` (release/orc CBOR build). Numbers captured in `doc/bench_baseline.md` § "Event dispatch — Part D-6". Headlines on Apple M4 / macOS / Nim 2.2.4 / `-d:release --mm:orc`: (a) no_foreign_no_nim **615 ns/emit** (atomic-counter fast path), (b) one_foreign **976 ns** (+361 ns courier path overhead), (c) many_foreign (8 cbs) **895 ns** (per-callback marginal cost ≈ 0; encode-once amortizes), (d) nim_only_same_thread (4) **1170 ns** (Lane 1 cost). | 0.5 d |
| 7 | **D-7 Doc sweep** [shipped] | Updated `doc/CBOR_Refactoring_Round2.md` §3 Part D heading + sequencing-table row to `[shipped]` with a note pointing here. Rewrote `doc/FFI_API.md` "Delivery thread / Why two threads / Startup ordering / Event behavior" section to reflect the as-shipped three-lane design (atomic-counter fast path + courier ring + delivery-thread fan-out + `SubsRegistry` direct-write subscribe path) and updated the Mermaid sequence diagram. Refreshed `AGENTS.md` two-thread description with the same. Marked this doc IMPLEMENTED with the commit chain inline. | 0.5 d |
