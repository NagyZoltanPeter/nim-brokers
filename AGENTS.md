...
The Rust wrapper has **full parity** with the C++ and Python wrappers across the type matrix the typemappingtestlib parity test exercises: primitive scalars (bool / intN / uintN / byte / floatN / string), enums (`#[repr(i32)]` Rust enums with `From<i32>`), distinct/alias (`pub type X = Y`), `seq[primitive]` (`Vec<T>`), `seq[string]` (`Vec<String>`), `seq[Object]` and `seq[Object<seq>]` (`Vec<T>` with arbitrary nested fields), `array[N, T]` for primitive / string / Object element types, `Option[T]`, and inline-nested Objects. Associative containers `Table[string, V]` map to `HashMap<String, V>` — string-keyed tables are supported across C++/Rust/Go (`std::unordered_map` / `HashMap` / `map[string]V`); non-string keys (int8..64 / char / enum / distinct-of-scalar) currently round-trip only through the Python wrapper, because the foreign CBOR libraries do not convert the text-string wire keys back to a typed key — see `doc/design/ASSOC_CONTAINERS_IMPL_PLAN.md` §9b for the follow-up.
...
This project is indexed by GitNexus as **nim-brokers** (6522 symbols, 11468 relationships, 229 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
