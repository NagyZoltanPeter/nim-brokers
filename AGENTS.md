...
```
nim c -d:BrokerFfiApi --threads:on --app:lib --path:. --outdir:examples/ffiapi/nimlib/build -d:brokerDebug examples/ffiapi/nimlib/mylib.nim"
```
or with such command template:
```
nim c -d:BrokerFfiApi --threads:on --app:lib --path:. --outdir:build -d:brokerDebug <file-to-compile>
```
